...
class xmlDb
{
        ...
        if ($select != '*') {
            ...
            if ($this->join_tables) {
                foreach ($this->join_tables as $join_table) {
                    $columns = array_unique(array_merge($columns, $this->getColumns($join_table)));
                }
            }
        }
    ...
    private function get($results = 0)
    {
        ...
        if (!is_null($this->join_tables)) {
            foreach ($this->join_tables as $join_table) {
                array_push($jtable_columnss, $this->getColumns($join_table));
            }
        }

        //print('//database/' . $this->table . '/row' . $this->getQuery());
        //print_r($this->xml);

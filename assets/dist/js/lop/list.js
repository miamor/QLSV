...
function del (itemID) {
    ...
    if (itemID) {
        if (confirm("Are you sure want to remove item "+title+" permanently?")) {
            ...
            $.get(MAIN_URL+'/requests/lop/delete.php?id='+itemID, function (data) {
                console.log(data);
...
function update (itemID) {
    $('.the-board').load(MAIN_URL + '/lop/'+itemID+'?v=window', function (data) {
        popup();
...
$(document).ready(function () {
    table = $('#buyList').DataTable({
		"ajax": MAIN_URL+'/requests/lop/readAll.php',
		"columns": [
			{ "data": "name" },
			{ "data": "created",
    ...
    $('.btn-add').click(function () {
        $('.the-board').load(MAIN_URL + '/lop?mode=add&v=window', function (data) {
            popup();
            $('.popup-content [role="close"], .popup-inner > div:not(".popup-content")').click(function () {
                table.ajax.reload();
            })

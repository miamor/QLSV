...
function loadEdit() {
    ...
    $('.open-account').click(function () {
        $.post(MAIN_URL+'/requests/sinhvien/openaccount.php', {username: $('[name="username"]').val()}, function (data) {
            if (data == 1) {
                //location.reload();
    ...
    $('.edit').submit(function () {
        ...
        $.post(MAIN_URL+'/requests/sinhvien/edit.php', $(this).serialize(), function (data) {
            console.log(data);
            if (data == 1) {
                //location.reload();
                console.log(MAIN_URL+'/sinhvien/'+$('[name="username"]').val());
                mtip('', 'success', '', 'Cập nhật thông tin thành công');
                clearInterval(timeinterval);
                $('section.content > .edit').load(MAIN_URL+'/sinhvien/'+$('[name="username"]').val()+' section.content > .edit', function (data) {
                    //console.log(data);
                    loadEdit();
                });
            } else if (data == -1) {
